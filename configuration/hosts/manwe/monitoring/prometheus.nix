...
in {
  ...
  services.prometheus = {
    ...
    scrapeConfigs = let
      # nginx filtered
      filteredHosts = lib.filterAttrs
        (name: host: host ? prometheusExporters && host ? server && host.server)
        hosts;
      ...
      snmp_file =
        pkgs.writeText "snmp_targets.json" (builtins.toJSON snmp_targets);
      minecraft_targets = [
        # minecraft (mc-weimar.dev)
        {
          targets = [ "localhost:9225" ];
          labels = { server_name = "mc-weimar.dev"; };
        }
      ];
      minecraft_file = pkgs.writeText "minecraft_targets.json"
        (builtins.toJSON minecraft_targets);
    in [
      {
        job_name = "dummy";
