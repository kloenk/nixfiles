...
let
  ...
  /*languageDir = languages: pkgs.symlinkJoin {
    name = "wp-languages-dir";
    pathes = languages;
  };*/
  pkg = hostName: cfg: pkgs.stdenv.mkDerivation rec {
    ...
    installPhase = ''
      ...
      mkdir $out/share/wordpress/wp-content/languages
  ...
  siteOpts = { lib, name, ... }:
    {
      options = {
        ...
        languages = mkOption {
          ...
          default = [];
          description = ''
            List of path(s) to respective languages(s) which are copied from the 'languages' directory.
        ...
        virtualHost = mkOption {
          type = types.submodule (import (inputs.nixpkgs + "/nixos/modules/services/web-servers/apache-httpd/vhost-options.nix"));
